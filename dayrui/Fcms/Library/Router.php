...
class Router {
    ...
    public function search_url($params = [], $name = '', $value = '', $mid = '', $fid = SITE_FID) {
        ...
        if ($rule && $rule['search']) {
            ...
            return dr_url_rel(dr_url_prefix($this->get_url_value($data, $url, $this->url_prefix('rewrite', $mod)), $mod['dirname']));
        } else {
            return dr_url_rel(dr_url_prefix($this->url_prefix('php', $mod, [], $fid) . trim('c=search&' . (is_array($params) ? http_build_query($params) : ''), '&'), $mod['dirname']));
        }
    }
